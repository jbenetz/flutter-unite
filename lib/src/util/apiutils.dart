...
Future<ExplnationOfBenfits> getEOB(String accessToken, String patientId) async {
  ...
  //first 50
  var eobURL = HuMedStatics.CMS_BB_API_EOB_URI +
      "?patient=$patientId&startIndex=0&count=$maxPageCount";

  final response =
      await http.get(eobURL, headers: {"Authorization": authHeader});

  if (response.statusCode == 200) {
    eob = new ExplnationOfBenfits(new HuJSON(
        json.decode(response.body))); //build model object from the response.
    ...
    while (nextPageURL != null) {
      print(nextPageURL);
      final eobResponse =
          await http.get(nextPageURL, headers: {"Authorization": authHeader});
      if (eobResponse.statusCode == 200) {
        var moreEobs =
            new ExplnationOfBenfits(new HuJSON(json.decode(eobResponse.body)));
        eob.benefitEntries
            .addAll(moreEobs.benefitEntries); //accumulate benefit entries
    ...
    if (eob.benefitEntries != null && eob.benefitEntries.length > 0) {
      eob.benefitEntries
          .sort((entry1, entry2) => entry2.eobDate.compareTo(entry1.eobDate));
    }
...
testLoadAllData() {
  Future.wait([
    testDataLoad('data/pharma.json'),
    testDataLoad('data/eob.json'),
    testDataLoad('data/SampleClientEOB.json')
