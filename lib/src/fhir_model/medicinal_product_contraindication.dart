...
import 'medicinal_product_contraindication_other_therapy.dart';
...
class MedicinalProductContraindication {
    ...
    MedicinalProductContraindication.fromJSON( Map json ){
        this.id = json['id'];
        ...
        this.disease = json['disease'] == null ? null : FHIRObjectFactory.buildFromJSON('CodeableConcept', json['disease']);
        ...
        this.therapeuticIndication = List<Reference>();
        if (json['therapeuticIndication'] != null && json['therapeuticIndication'].length > 0) {
          if( json['therapeuticIndication'] is List ){
            json['therapeuticIndication'].forEach((i){
              this.therapeuticIndication.add(FHIRObjectFactory.buildFromJSON('Reference', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['therapeuticIndication'];
            this.therapeuticIndication.add(FHIRObjectFactory.buildFromJSON('Reference', i));
          }          
        }
        this.otherTherapy = List<MedicinalProductContraindicationOtherTherapy>();
        if (json['otherTherapy'] != null && json['otherTherapy'].length > 0) {
          if( json['otherTherapy'] is List ){
            json['otherTherapy'].forEach((i){
              this.otherTherapy.add(FHIRObjectFactory.buildFromJSON('MedicinalProductContraindicationOtherTherapy', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
