...
import 'research_element_definition_characteristic.dart';
...
class ResearchElementDefinition {
    ...
    ResearchElementDefinition.fromJSON( Map json ){
        this.id = json['id'];
        ...
        this.extComment = List<Element>();
        if (json['_comment'] != null && json['_comment'].length > 0) {
          if( json['_comment'] is List ){
            json['_comment'].forEach((i){
              this.extComment.add(FHIRObjectFactory.buildFromJSON('Element', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['_comment'];
        ...
        this.extVariableType = json['_variableType'] == null ? null : FHIRObjectFactory.buildFromJSON('Element', json['_variableType']);
        this.characteristic = List<ResearchElementDefinitionCharacteristic>();
        if (json['characteristic'] != null && json['characteristic'].length > 0) {
          if( json['characteristic'] is List ){
            json['characteristic'].forEach((i){
              this.characteristic.add(FHIRObjectFactory.buildFromJSON('ResearchElementDefinitionCharacteristic', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
