...
class Library {
    ...
    Library.fromJSON( Map json ){
        ...
        this.parameter = List<ParameterDefinition>();
        if (json['parameter'] != null && json['parameter'].length > 0) {
          if( json['parameter'] is List ){
            json['parameter'].forEach((i){
              this.parameter.add(FHIRObjectFactory.buildFromJSON('ParameterDefinition', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
        ...
        this.content = List<Attachment>();
        if (json['content'] != null && json['content'].length > 0) {
          if( json['content'] is List ){
            json['content'].forEach((i){
              this.content.add(FHIRObjectFactory.buildFromJSON('Attachment', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['content'];
