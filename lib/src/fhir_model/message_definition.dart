...
import 'coding.dart';
import 'message_definition_focus.dart';
import 'message_definition_allowed_response.dart';
...
class MessageDefinition {
    ...
    MessageDefinition.fromJSON( Map json ){
        this.id = json['id'];
        ...
        this.focus = List<MessageDefinitionFocus>();
        if (json['focus'] != null && json['focus'].length > 0) {
          if( json['focus'] is List ){
            json['focus'].forEach((i){
              this.focus.add(FHIRObjectFactory.buildFromJSON('MessageDefinitionFocus', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['focus'];
        ...
        this.responseRequired = json['responseRequired'];
        this.extResponseRequired = json['_responseRequired'] == null ? null : FHIRObjectFactory.buildFromJSON('Element', json['_responseRequired']);
        this.allowedResponse = List<MessageDefinitionAllowedResponse>();
        if (json['allowedResponse'] != null && json['allowedResponse'].length > 0) {
          if( json['allowedResponse'] is List ){
            json['allowedResponse'].forEach((i){
              this.allowedResponse.add(FHIRObjectFactory.buildFromJSON('MessageDefinitionAllowedResponse', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['allowedResponse'];
            this.allowedResponse.add(FHIRObjectFactory.buildFromJSON('MessageDefinitionAllowedResponse', i));
          }          
        }
        this.graph = List<String>();
        if (json['graph'] != null && json['graph'].length > 0) {
          if( json['graph'] is List ){
            json['graph'].forEach((i){
              this.graph.add(i);
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
