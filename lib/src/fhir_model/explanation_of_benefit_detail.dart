...
import 'explanation_of_benefit_sub_detail.dart';
...
class ExplanationOfBenefitDetail {
    ...
    ExplanationOfBenefitDetail.fromJSON( Map json ){
        this.id = json['id'];
        ...
        this.subDetail = List<ExplanationOfBenefitSubDetail>();
        if (json['subDetail'] != null && json['subDetail'].length > 0) {
          if( json['subDetail'] is List ){
            json['subDetail'].forEach((i){
              this.subDetail.add(FHIRObjectFactory.buildFromJSON('ExplanationOfBenefitSubDetail', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
