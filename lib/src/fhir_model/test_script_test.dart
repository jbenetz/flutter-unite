...
import 'element.dart';
import 'test_script_action1.dart';
...
class TestScriptTest {
    ...
    TestScriptTest.fromJSON( Map json ){
        this.id = json['id'];
        ...
        this.action = List<TestScriptAction1>();
        if (json['action'] != null && json['action'].length > 0) {
          if( json['action'] is List ){
            json['action'].forEach((i){
              this.action.add(FHIRObjectFactory.buildFromJSON('TestScriptAction1', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['action'];
