...
class StructureDefinitionDifferential {
    ...
    StructureDefinitionDifferential.fromJSON( Map json ){
        this.id = json['id'];
        ...
        this.element = List<ElementDefinition>();
        if (json['element'] != null && json['element'].length > 0) {
          if( json['element'] is List ){
            json['element'].forEach((i){
              this.element.add(FHIRObjectFactory.buildFromJSON('ElementDefinition', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['element'];
