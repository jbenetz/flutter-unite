...
import 'risk_evidence_synthesis_sample_size.dart';
import 'risk_evidence_synthesis_risk_estimate.dart';
import 'risk_evidence_synthesis_certainty.dart';
...
class RiskEvidenceSynthesis {
    ...
    RiskEvidenceSynthesis.fromJSON( Map json ){
        this.id = json['id'];
        ...
        this.sampleSize = json['sampleSize'] == null ? null : FHIRObjectFactory.buildFromJSON('RiskEvidenceSynthesisSampleSize', json['sampleSize']);
        this.riskEstimate = json['riskEstimate'] == null ? null : FHIRObjectFactory.buildFromJSON('RiskEvidenceSynthesisRiskEstimate', json['riskEstimate']);
        this.certainty = List<RiskEvidenceSynthesisCertainty>();
        if (json['certainty'] != null && json['certainty'].length > 0) {
          if( json['certainty'] is List ){
            json['certainty'].forEach((i){
              this.certainty.add(FHIRObjectFactory.buildFromJSON('RiskEvidenceSynthesisCertainty', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
