...
import 'structure_map_structure.dart';
import 'structure_map_group.dart';
...
class StructureMap {
    ...
    StructureMap.fromJSON( Map json ){
        this.id = json['id'];
        ...
        this.structure = List<StructureMapStructure>();
        if (json['structure'] != null && json['structure'].length > 0) {
          if( json['structure'] is List ){
            json['structure'].forEach((i){
              this.structure.add(FHIRObjectFactory.buildFromJSON('StructureMapStructure', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['structure'];
            this.structure.add(FHIRObjectFactory.buildFromJSON('StructureMapStructure', i));
          }          
        }
        this.import = List<String>();
        if (json['import'] != null && json['import'].length > 0) {
          if( json['import'] is List ){
            json['import'].forEach((i){
              this.import.add(i);
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
        ...
        this.group = List<StructureMapGroup>();
        if (json['group'] != null && json['group'].length > 0) {
          if( json['group'] is List ){
            json['group'].forEach((i){
              this.group.add(FHIRObjectFactory.buildFromJSON('StructureMapGroup', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['group'];
