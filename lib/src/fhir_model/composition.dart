...
import 'composition_attester.dart';
import 'composition_relates_to.dart';
import 'composition_event.dart';
import 'composition_section.dart';
...
class Composition {
    ...
    Composition.fromJSON( Map json ){
        ...
        this.extConfidentiality = json['_confidentiality'] == null ? null : FHIRObjectFactory.buildFromJSON('Element', json['_confidentiality']);
        this.attester = List<CompositionAttester>();
        if (json['attester'] != null && json['attester'].length > 0) {
          if( json['attester'] is List ){
            json['attester'].forEach((i){
              this.attester.add(FHIRObjectFactory.buildFromJSON('CompositionAttester', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['attester'];
        ...
        this.relatesTo = List<CompositionRelatesTo>();
        if (json['relatesTo'] != null && json['relatesTo'].length > 0) {
          if( json['relatesTo'] is List ){
            json['relatesTo'].forEach((i){
              this.relatesTo.add(FHIRObjectFactory.buildFromJSON('CompositionRelatesTo', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
        ...
        this.event = List<CompositionEvent>();
        if (json['event'] != null && json['event'].length > 0) {
          if( json['event'] is List ){
            json['event'].forEach((i){
              this.event.add(FHIRObjectFactory.buildFromJSON('CompositionEvent', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['event'];
        ...
        if (json['section'] != null && json['section'].length > 0) {
          if( json['section'] is List ){
            json['section'].forEach((i){
              this.section.add(FHIRObjectFactory.buildFromJSON('CompositionSection', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['section'];
