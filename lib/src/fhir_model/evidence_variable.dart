...
import 'evidence_variable_characteristic.dart';
...
class EvidenceVariable {
    ...
    EvidenceVariable.fromJSON( Map json ){
        this.id = json['id'];
        ...
        this.characteristic = List<EvidenceVariableCharacteristic>();
        if (json['characteristic'] != null && json['characteristic'].length > 0) {
          if( json['characteristic'] is List ){
            json['characteristic'].forEach((i){
              this.characteristic.add(FHIRObjectFactory.buildFromJSON('EvidenceVariableCharacteristic', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
