...
import 'explanation_of_benefit_detail1.dart';
...
class ExplanationOfBenefitAddItem {
    ...
    ExplanationOfBenefitAddItem.fromJSON( Map json ){
        this.id = json['id'];
        ...
        this.detail = List<ExplanationOfBenefitDetail1>();
        if (json['detail'] != null && json['detail'].length > 0) {
          if( json['detail'] is List ){
            json['detail'].forEach((i){
              this.detail.add(FHIRObjectFactory.buildFromJSON('ExplanationOfBenefitDetail1', i));
            });
          } else {
            // this handles the case when the list only has one object and does not return as a list
            var i = json['detail'];
